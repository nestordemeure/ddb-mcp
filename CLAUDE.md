...
- **No API key is needed today**, but that may be an unenforced gate rather than policy. `DDB_API_KEY` is sent when set, so the client keeps working if enforcement is switched on. Keys are free and carry no affiliation requirement: DDB's documentation states that any registered user can generate one from the *Meine DDB* area of their account, at <https://www.deutsche-digitale-bibliothek.de/user/apikey>, and there is no paid tier. The signup pages are on the Anubis-walled `www` host, so that flow has only been read about, not walked through.
